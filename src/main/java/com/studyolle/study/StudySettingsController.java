...
public class StudySettingsController {
    ...
    @PostMapping("/banner")
    public String studyBannerSubmit(@CurrentUser Account account, @PathVariable String path, String image, RedirectAttributes attributes) {
        Study study = studyService.getStudyToUpdate(account, path);
    ...
    public String updateStudyPath(@CurrentUser Account account, @PathVariable String path, @RequestParam String newPath,
        ...
        Study study = studyService.getStudyToUpdateStatus(account, path);
        if (!studyService.isValidPath(newPath)) { // @Valid 대신 Service에서 검증
            model.addAttribute(account);
            model.addAttribute(study);
